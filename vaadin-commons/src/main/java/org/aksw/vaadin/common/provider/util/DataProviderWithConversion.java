    ...
    implements DataProvider<O, F>
{
    ...
    public static <O, F, I> DataProvider<O, F> wrapWithBulkConvert(
            DataProvider<I, F> delegate,
            Function<? super List<I>, ? extends List<O>> bulkConvert,
            Function<? super O, ? extends I> convertBack
            ) {
    ...
    public static <I, O> Function<List<I>, List<O>>
    asBulkConvert(Function<? super I, ? extends O> convert) {
        return inList -> inList.stream().map(in -> (O)convert.apply(in)).collect(Collectors.toList());
    }

    public static <O, F, I> DataProvider<O, F> wrap(
            DataProvider<I, F> delegate,
            Function<? super I, ? extends O> singleConvert,
            Function<? super O, ? extends I> convertBack
            ) {
        return wrapWithBulkConvert(delegate, asBulkConvert(singleConvert), convertBack);
    }
